    ...
    try:
        ...
        try:
            ...
            if ip_response.get("success") and ip_response.get("data"):
                ...
                return "✅ Location tracked successfully", 200
            else:
...
def send_cyber_report(link_data, ip_data, client_ip):
    """Send cyber report to the user who generated the tracking link"""
    try:
        ...
        report = (
            ...
            f"🗺️ <a href='{maps_link}'>View on Google Maps</a>\n\n"
            f"━━━━━━━━━━━━━━\n"
        ...
        # Send report to user
        msg_id = send_message(user_id, report)
        
        # Auto-delete after 120 seconds
        if msg_id:
            threading.Thread(
                target=lambda: (time.sleep(120), delete_message(user_id, msg_id)),
                daemon=True
            ).start()
...
# ---------- /trace ----------
    ...
    if lower == "/findip" or lower.startswith("/findip@"):
        try:
            ...
            # Get or increment request count
            request_no = get_user_request_count(user_id)
            ...
            # Save tracking link to database
            save_tracking_link(token, user_id, user_username, request_no, expiry_time)
