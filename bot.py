...
def process_message(chat_id, text, user_id, message_id):
    ...
    if lower.startswith("/blockkey "):
        ...
        if not clean_text:
            send_message(chat_id, "❌ Please provide key\n💡 Example: /blockkey DTX-XXXX", reply_to_message_id=reply_to)
            return
        
        block_key(clean_text)
        send_message(chat_id, f"✅ Key blocked: {clean_text}", reply_to_message_id=reply_to)
        return
    ...
    if lower.startswith("/aadhaar "):
        ...
        try:
            ...
            if not r:
                ...
                send_message(chat_id, "⚠️ No record found", reply_to_message_id=reply_to)
                return
            fid = send_txt_file_with_caption(chat_id, f"Report_{datetime.now().strftime('%d%m%Y_%H%M%S')}.txt", build_common_txt(r[0]), reply_to_message_id=reply_to)
            delete_message(chat_id, loading)
    ...
    if lower.startswith("/gst "):
        ...
        try:
            ...
            if not d:
                ...
                send_message(chat_id, "⚠️ No record found", reply_to_message_id=reply_to)
                return
            fid = send_txt_file_with_caption(chat_id, f"Report_{datetime.now().strftime('%d%m%Y_%H%M%S')}.txt", build_gst_txt(d), reply_to_message_id=reply_to)
            delete_message(chat_id, loading)
            threading.Thread(target=auto_delete_file, args=(chat_id, fid), daemon=True).start()
        except:
            delete_message(chat_id, loading)
    ...
    if lower.startswith("/ifsc "):
        ...
        try:
            ...
            if not d:
                ...
                send_message(chat_id, "⚠️ No record found", reply_to_message_id=reply_to)
                return
            fid = send_txt_file_with_caption(chat_id, f"Report_{datetime.now().strftime('%d%m%Y_%H%M%S')}.txt", build_ifsc_txt(d), reply_to_message_id=reply_to)
            delete_message(chat_id, loading)
            threading.Thread(target=auto_delete_file, args=(chat_id, fid), daemon=True).start()
        except:
            delete_message(chat_id, loading)
    ...
    if lower.startswith("/fam "):
        ...
        try:
            ...
            if not d:
                ...
                send_message(chat_id, "⚠️ No record found", reply_to_message_id=reply_to)
                return
            fid = send_txt_file_with_caption(chat_id, f"Report_{datetime.now().strftime('%d%m%Y_%H%M%S')}.txt", build_fam_txt(d), reply_to_message_id=reply_to)
            delete_message(chat_id, loading)
            threading.Thread(target=auto_delete_file, args=(chat_id, fid), daemon=True).start()
        except:
            delete_message(chat_id, loading)
