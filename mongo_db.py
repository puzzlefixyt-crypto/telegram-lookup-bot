...
ip_tracking_collection = db["ip_tracking"]  # NEW COLLECTION FOR IP TRACKING
...
# ================= IP TRACKING (NEW) =================
def save_ip_tracking_request(request_no, user_id, username, generation_time, expiry_time):
    """Save IP tracking request to MongoDB"""
    ip_tracking_collection.insert_one({
        "request_no": request_no,
        "generator_user_id": user_id,
        "generator_username": username,
        "generation_time": generation_time,
        "expiry_time": expiry_time,
        "status": "pending"
    })

def get_ip_tracking_request(request_no):
    """Get IP tracking request by request number"""
    return ip_tracking_collection.find_one({"request_no": request_no})

def mark_ip_tracking_completed(request_no):
    """Mark IP tracking request as completed"""
    ip_tracking_collection.update_one(
        {"request_no": request_no},
