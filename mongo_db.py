...
available_keys_collection = db["available_keys"]
...
def load_available_keys():
    """Load available keys from MongoDB and return as a dictionary"""
    doc = available_keys_collection.find_one({"_id": "available_keys"})
    if doc and "data" in doc:
        return doc["data"]
    return {}

def save_available_keys(data):
    """Save available keys dictionary to MongoDB"""
    available_keys_collection.update_one(
        {"_id": "available_keys"},
        {"$set": {"data": data}},
        upsert=True
    )
